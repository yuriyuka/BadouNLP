...
class SiameseNetwork(nn.Module):
    ...
    def cosine_triplet_loss(self, a, p, n, margin=None):
        ...
        loss = torch.mean(torch.relu(diff))  # 使用 relu 确保梯度传递
        return loss
    ...
    def forward(self, sentence1, sentence2=None, sentence3=None):
        if sentence2 is not None and sentence3 is not None:
            # 训练模式：计算 Triplet Loss
            a = self.sentence_encoder(sentence1)
            p = self.sentence_encoder(sentence2)
            n = self.sentence_encoder(sentence3)
