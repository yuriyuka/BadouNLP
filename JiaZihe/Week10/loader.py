...
class DataGenerator:
    ...
    def _mask_input(self, input_ids):
        ...
        labels = input_ids.copy()  # 原始标签
        mask_indices = torch.rand(len(input_ids)) < 0.15  # 15%的token

        for i, masked in enumerate(mask_indices):
            if masked and input_ids[i] not in [self.config["pad_idx"],
    ...
    def prepare_data(self, title, content):
        ...
        masked_input, gold = self._mask_input(input_seq)

        # gold = self.encode_sentence(title, self.config["output_max_length"], False, True) #不进入模型，用于计算loss

        self.data.append([torch.LongTensor(masked_input),
