...
class Evaluator:
    ...
    def knwb_to_vector(self):
        ...
        for standard_question_index, question_ids in self.train_data.dataset.knwb.items():
            # print(f"standard_question_index:{standard_question_index}---question_ids:{question_ids}")
        ...
        with torch.no_grad():
            ...
            # print(f"knwb_vectors1:{self.knwb_vectors}")
            #将所有向量都作归一化 v / |v|
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            # print(f"batch_data:{batch_data}----index:{index}")
            if torch.cuda.is_available():
                batch_data = [d.cuda() for d in batch_data]
            input_id, labels = batch_data #输入变化时这里需要修改，比如多输入，多输出的情况
            # print(f"input_id:{input_id}----labels:{labels}")
            with torch.no_grad():
    ...
    def write_stats(self, test_question_vectors, labels):
        # print(f"test_question_vectors:{test_question_vectors}----labels:{labels}")
