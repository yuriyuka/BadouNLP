#!/usr/bin/env python3
#coding: utf-8

# 基于训练好的词向量模型进行聚类
# 聚类采用Kmeans算法，并按类内平均距离排序
import math
import re
import json
import jieba
import numpy as np
from gensim.models import Word2Vec
from sklearn.cluster import KMeans
from collections import defaultdict

# 输入模型文件路径
# 加载训练好的模型
def load_word2vec_model(path):
    model = Word2Vec.load(path)
    return model

def load_sentence(path):
    sentences = set()
    with open(path, encoding="utf8") as f:
        for line in f:
            sentence = line.strip()
            sentences.add(" ".join(jieba.cut(sentence)))
    print("获取句子数量：", len(sentences))
    return sentences

# 将文本向量化
def sentences_to_vectors(sentences, model):
    vectors = []
    for sentence in sentences:
        words = sentence.split()  # sentence是分好词的，空格分开
        vector = np.zeros(model.vector_size)
        # 所有词的向量相加求平均，作为句子向量
        for word in words:
            try:
                vector += model.wv[word]
            except KeyError:
                # 部分词在训练中未出现，用全0向量代替
                vector += np.zeros(model.vector_size)
        vectors.append(vector / len(words))
    return np.array(vectors)

def main():
    model = load_word2vec_model(r"E:\学习过程\week5 词向量及文本向量\model.w2v") # 加载词向量模型
    sentences = load_sentence("titles.txt")  # 加载所有标题
    vectors = sentences_to_vectors(sentences, model)   # 将所有标题向量化

    n_clusters = int(math.sqrt(len(sentences)))  # 指定聚类数量
    print("指定聚类数量：", n_clusters)
    kmeans = KMeans(n_clusters)  # 定义一个kmeans计算类
    kmeans.fit(vectors)          # 进行聚类计算

    # 计算每个聚类的类内平均距离
    cluster_distances = defaultdict(list)
    for sentence, label, vector in zip(sentences, kmeans.labels_, vectors):
        # 计算样本到聚类中心的距离
        distance = np.linalg.norm(vector - kmeans.cluster_centers_[label])
        cluster_distances[label].append(distance)

    # 计算每个聚类的平均距离
    cluster_avg_distances = {}
    for label, distances in cluster_distances.items():
        cluster_avg_distances[label] = np.mean(distances)

    # 按平均距离排序聚类
    sorted_clusters = sorted(cluster_avg_distances.items(), key=lambda x: x[1])

    # 输出排序后的聚类结果
    for label, avg_distance in sorted_clusters:
        print(f"cluster {label}, 平均类内距离: {avg_distance:.4f}:")
        cluster_sentences = [sentence.replace(" ", "") for sentence, lbl in zip(sentences, kmeans.labels_) if lbl == label]
        for i in range(min(10, len(cluster_sentences))):  # 打印每个聚类的前10个句子
            print(cluster_sentences[i])
        print("---------")

if __name__ == "__main__":
    main()
