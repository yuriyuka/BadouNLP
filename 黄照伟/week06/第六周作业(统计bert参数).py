...
bert = BertModel.from_pretrained(r"/Library/workerspace/python_test/badou_demo1/week06/bert-base-chinese", return_dict=False)
...
vocab_size = db.word_embeddings.shape[0]

hidden_size = db.hidden_size

max_position_size = db.position_embeddings.shape[0]
token_type_embeddings = db.token_type_embeddings.shape[0]

print("vocab_size:", vocab_size)
print("hidden_size:", hidden_size)
print("max_position_size:", max_position_size)
print("token_type_embeddings:", token_type_embeddings)


#embedding
embedding_total = (vocab_size + max_position_size + token_type_embeddings) * hidden_size
print("embedding_total:", embedding_total)

# multi-heads
head_size = db.num_attention_heads
# 3: Q,K,V 数量为3
self_head_size = hidden_size * (hidden_size/head_size) * 3
multi_heads_size = self_head_size * head_size + (hidden_size * hidden_size)
print("multi_heads_size:", multi_heads_size)

dmodel_size = hidden_size
# 全连接层 FeedForward
fnn_size = dmodel_size * 4 * hidden_size * 2
# LayerNorm
layer_norm_size = hidden_size * 2+ hidden_size*2*2
#获取encoder 的层数
num_layers_size =  db.num_layers

# embedding + multi-heads + FNN +
total_params = embedding_total + num_layers_size * ( multi_heads_size + fnn_size + layer_norm_size)
print(f"Total parameters: {total_params / 1e6:.2f}M")  # 输出参数总量（百万级）
