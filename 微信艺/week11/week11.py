...
def build_dataset(tokenizer, corpus, max_length, batch_size):
    ...
    for i, (prompt, answer) in enumerate(corpus):
        ...
        x = [tokenizer.cls_token_id] + prompt_encode + [
            tokenizer.sep_token_id] + answer_encode + [tokenizer.sep_token_id]
        y = len(prompt_encode) * [-1] + [-1] + answer_encode + [
...
def main(corpus_path, save_weight=True):
    ...
    train_data = build_dataset(tokenizer, corpus, max_length,
                               batch_size)  # 建立数据集
    ...
    for epoch in range(epoch_num):
        ...
        print(generate_sentence("北京明年拟推工作日半价观看电影", model,
                                tokenizer))
