...
class DataGenerator:
    ...
    def random_train_sample(self):
        ...
        """三元组采样样本生成"""
        # 随机选择锚点类别
        anchor_class = random.choice(list(self.knwb.keys()))
        # 确保锚点类别有足够样本
        while len(self.knwb[anchor_class]) < 2:
            anchor_class = random.choice(list(self.knwb.keys()))
        ...
        # 随机选择不同类别的负样本
        # 获取所有其他类别
        other_classes = [cls for cls in self.knwb.keys() if cls != anchor_class]
        # 随机选择一个负样本类别
        neg_class = random.choice(other_classes)
        # 从负样本类别中随机选取一个负样本
        negative = random.choice(self.knwb[neg_class])
