...
class DataGenerator:
    ...
    def load(self):
        ...
        with open(self.path, encoding="utf8") as f:
            for line in f:
                ...
                #加载训练集
                if isinstance(line, dict):#判断是否字典类型
                    ...
                    for question in questions:
                        ...
                        self.knwb[self.schema[label]].append(input_id)#{target_num:[[sen vec],[s1 vec],[s3 vec]]}
                #加载测试集
                else:
                    self.data_type = "test"
                    ...
                    self.data.append([input_id, label_index])#[[senvec],target_number]
        return
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            return self.random_train_sample() #随机生成一个训练样本,1个[[s1 vec],[s2 vec],0/1]
        #!!!!这里就学会了哈，要使用data_loader那个，每次只需返回一个用例就行
        else:
            return self.data[index]#[[senvec],target_number]
