...
# from docx import Document as DocxDocument  # 导入python-docx的Document，并重命名避免冲突
# from langchain_core.documents import Document as LangChainDocument  # 导入LangChain的Document
#
#
# def load_docx_with_tables(file_path):
#     # 1. 用python-docx的Document加载Word文件（这个才有paragraphs属性）
#     docx_doc = DocxDocument(file_path)  # 注意这里用的是DocxDocument（python-docx的类）
#     langchain_docs = []  # 存储LangChain的Document对象
#
#     # 2. 提取段落文本
#     for para in docx_doc.paragraphs:  # 这里的docx_doc是python-docx的对象，有paragraphs属性
#         if para.text.strip():  # 跳过空段落
#             langchain_docs.append(
#                 LangChainDocument(  # 用LangChain的Document封装内容
...
#
#     # 3. 提取表格内容
#     for table in docx_doc.tables:  # 同理，tables也是python-docx对象的属性
#         table_text = []
...
#
# # 使用示例
# docs = load_docx_with_tables("淘宝网超时说明.docx")
# print(f"成功提取 {len(docs)} 个内容块（段落+表格）")
# for i, doc in enumerate(docs, 1):  # 从1开始计数
#     print(f"内容块 {i}（类型：{doc.metadata['type']}）：")
#     print("-" * 50)
