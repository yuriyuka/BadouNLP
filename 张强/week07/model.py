...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        # 添加标志区分基础BERT和自定义BERT模型
        self.is_base_bert = config["model_type"] == "bert"
    ...
    # def forward(self, x, attention_mask=None,target=None):
        if self.use_bert:  # bert返回的结果是 (sequence_output, pooler_output)
            ...
            #
            # # 基础BERT直接使用pooler_output
            # if self.is_base_bert:
            #     x = outputs[1]  # 形状: (batch_size, hidden_size)
            # else:
            #     # 自定义BERT模型使用序列输出
            #     x = outputs[0]
        else:
        ...
        # if self.use_bert and not self.is_base_bert or not self.use_bert:
        #     # 确保x是3D张量 (batch_size, features, seq_len)
        #     if x.dim() == 2:
