...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            return self.generate_triplet() #随机生成一个训练样本
        else:
    ...
    def generate_triplet(self):
        ...
        # 确保锚点类别有足够样本
        if len(self.knwb[anchor_category]) < 2:
            return self.generate_triplet()  # 递归直到找到合适类别

        # 从同一类别中选择锚点和正样本
        anchor, positive = random.sample(self.knwb[anchor_category], 2)

        # 选择负样本类别（与锚点不同）
        negative_category = random.choice(categories)
...
if __name__ == "__main__":
    ...
    dg = DataGenerator("../week8 文本匹配问题/data/data.json", Config)
    print(dg[0])