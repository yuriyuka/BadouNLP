...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.classify = nn.Linear(64, sentence_length+1)
...
def predict(model_path, vocab_path, input_strings):
    ...
    with torch.no_grad():  #不计算梯度
        ...
        predicted_classes = torch.argmax(result, dim=1)#模型预测
    for i, input_string in enumerate(input_strings):
        pred = predicted_classes[i].item()
        position = "未出现" if pred == sentence_length else f"位置 {pred}"
        print(f"输入：{input_string}, 'a'出现的位置：{position}")
