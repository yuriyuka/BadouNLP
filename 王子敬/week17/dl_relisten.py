...
class DialogueSystem:
    def __init__(self):
        self.load()
        self.repeat_keywords = {"没听清", "再说一遍", "重复", "什么"}

    def load(self):
        self.all_node_info = {}
        self.load_scenario(os.path.join("scenario", "scenario-买衣服.json"))
        self.load_scenario(os.path.join("scenario", "scenario-看电影.json"))
        self.load_slot_templet(os.path.join("scenario", "slot_fitting_templet.xlsx"))
    ...
    def nlu(self, memory):
        ...
        memory['is_repeat'] = False  # 标记不是重听请求
    ...
    def check_repeat_request(self, query):
        ...
        # 如果有任何重听关键词匹配，返回True
        return len(query_words & self.repeat_keywords) > 0
    ...
    def nlg(self, memory):
        # 如果是重听请求，返回上一轮回复
        if memory.get('is_repeat'):
            memory["response"] = memory.get('last_response', '抱歉，我没有可重复的内容')
            return memory
