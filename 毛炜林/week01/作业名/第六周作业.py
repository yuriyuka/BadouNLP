...
bert = BertModel.from_pretrained(r"F:\1\八斗精品班\第六周 语言模型\bert-base-chinese\bert-base-chinese", return_dict=False)
...
class DiyBert:
    ...
    def __init__(self, state_dict):
        ...
        self.total_parameters = self.calculate_total_parameters()
    ...
    def calculate_total_parameters(self):
        ...
        # 计算每个Transformer层的参数，然后乘以层数
        single_transformer_params = 0

        # Self-Attention参数
        attention_params = (
                self.transformer_weights[0][0].size + self.transformer_weights[0][1].size +  # Q
                self.transformer_weights[0][2].size + self.transformer_weights[0][3].size +  # K
                self.transformer_weights[0][4].size + self.transformer_weights[0][5].size +  # V
                self.transformer_weights[0][6].size + self.transformer_weights[0][7].size  # 注意力输出
        )

        # 前馈网络参数
        ff_params = (
                self.transformer_weights[0][10].size + self.transformer_weights[0][11].size +  # 中间层
                self.transformer_weights[0][12].size + self.transformer_weights[0][13].size  # 输出层
        )

        # LayerNorm参数
        ln1_params = self.transformer_weights[0][8].size + self.transformer_weights[0][9].size
        ln2_params = self.transformer_weights[0][14].size + self.transformer_weights[0][15].size

        single_transformer_params = attention_params + ff_params + ln1_params + ln2_params
        transformer_params = single_transformer_params * self.num_layers
        total_params += transformer_params

        # 计算Pooler层参数
        pooler_params = self.pooler_dense_weight.size + self.pooler_dense_bias.size
        ...
        self.param_stats = {
            ...
            'single_transformer': single_transformer_params,
            'attention': attention_params * self.num_layers,
            'feed_forward': ff_params * self.num_layers,
            'layer_norm': (ln1_params + ln2_params) * self.num_layers,
...
print("=" * 50)
print("BERT模型参数量计算结果:")
print(f"总参数量: {db.total_parameters:,}")
print(f"总参数量(百万): {db.total_parameters / 1000000:.2f}M")

# 打印参数组成详情
print("\n参数组成详情:")
print(
    f"Embedding层参数量: {db.param_stats['embedding']:,} ({db.param_stats['embedding'] / db.total_parameters * 100:.2f}%)")
print(
    f"Transformer层总参数量: {db.param_stats['transformer']:,} ({db.param_stats['transformer'] / db.total_parameters * 100:.2f}%)")
print(f"Pooler层参数量: {db.param_stats['pooler']:,} ({db.param_stats['pooler'] / db.total_parameters * 100:.2f}%)")

# 打印注意力机制和前馈网络参数量
print("\n核心组件参数量:")
print(
    f"注意力机制总参数量: {db.param_stats['attention']:,} ({db.param_stats['attention'] / db.total_parameters * 100:.2f}%)")
print(
    f"前馈网络总参数量: {db.param_stats['feed_forward']:,} ({db.param_stats['feed_forward'] / db.total_parameters * 100:.2f}%)")
print(
    f"LayerNorm总参数量: {db.param_stats['layer_norm']:,} ({db.param_stats['layer_norm'] / db.total_parameters * 100:.2f}%)")

# 每层详细参数
print("\n每层参数详情:")
print(f"单层Transformer参数量: {db.param_stats['single_transformer']:,}")
print(f"- 注意力机制参数量: {db.param_stats['attention'] / db.num_layers:,}")
print(f"- 前馈网络参数量: {db.param_stats['feed_forward'] / db.num_layers:,}")
print(f"- LayerNorm参数量: {db.param_stats['layer_norm'] / db.num_layers:,}")
