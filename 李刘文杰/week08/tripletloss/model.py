...
class SiameseNetwork(nn.Module):
    ...
    def cosine_triplet_loss(self, a, p, n, margin=None):
        ...
        # 如果没有设置margin，则将diff设置为ap - an + 0.1
        if margin is None:
            diff = ap - an + 0.1
        # 如果设置了margin，则将diff设置为ap - an + margin.squeeze()
        else:
            diff = ap - an + margin.squeeze()
        # 返回diff中大于0的部分的平均值
        return torch.mean(diff[diff.gt(0)]) #greater than 0

    #sentence : (batch_size, max_length) 涉及三种情况：1.同时传入两个句子，计算loss；2.单独传入一个句子，使用向量化能力；3.无标签，计算余弦距离
    def forward(self, sentence1, sentence2=None, sentence3=None):
        #同时传入3个句子,计算三元组loss
        if sentence2 is not None and sentence3 is not None:
            #sentence1, sentence2, sentence3分别表示anchor, positive, negative
