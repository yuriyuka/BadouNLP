...
class DataGenerator:
    def __init__(self, data_path, config):
        ...
        self.load()
    # TODO 生成三元组数据
    def triplet_train(self):
        standard_question_index = list(self.knwb.keys())
        p, n = random.sample(standard_question_index, 2)
        if len(self.knwb[p]) < 2:
            return self.triplet_train()
        if len(self.knwb[p]) >= 2:
            positive, anchor = random.sample(self.knwb[p], 2)
            negative = random.choice(self.knwb[n])
            return [anchor, positive, negative]
        else:
            positive, anchor = random.sample(self.knwb[p], 2)
            negative = random.choice(self.knwb[p])
            return [anchor, positive, negative]
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            # return self.random_train_samplein_sample() #随机生成一个训练样本
            # TODO 新增三元组生成器
            return self.triplet_train()
        else:
