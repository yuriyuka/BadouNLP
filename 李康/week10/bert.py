...
def build_sample(window_size, corpus, max_length):
    ...
    mask = [1] * len(x) + [0] * padding_size
    x = x[:max_length] + [0] * padding_size
    y = y[:max_length] + [0] * padding_size
    return x, y, mask
...
#window_size 样本长度
#corpus 语料字符串
def build_dataset(sample_length, window_size, corpus, max_length):
    dataset_x = []
    dataset_y = []
    dataset_mask = []
    for i in range(sample_length):
        x, y, mask = build_sample(window_size, corpus, max_length)
...
def generate_sentence(openings, model, window_size):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            openings += pred_char
            x = tokenizer.encode(openings[-window_size:], padding="max_length")
...
def train(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            x, y, mask = build_dataset(batch_size, window_size, corpus, max_length) #构建一组训练样本
            if torch.cuda.is_available():
                x, y = x.cuda(), y.cuda()
