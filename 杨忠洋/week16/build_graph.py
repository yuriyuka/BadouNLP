...
#连接图数据库
graph = Graph("bolt://127.0.0.1:7687", user="neo4j", password="yzyzzz999", name="neo4j")
...
for i, entity in enumerate(attribute_data):
    ...
    if entity in label_data:
        ...
        #带标签的实体构建语句
        temp_cypher = "CREATE (%s:%s %s)" % (entity, label, text) + "\n"
        cypher += "CREATE (%s:%s %s)" % (entity, label, text) + "\n"
    else:
        #不带标签的实体构建语句
        temp_cypher = "CREATE (%s %s)" % (entity, text) + "\n"
...
for i, head in enumerate(relation_data):
    ...
    if head not in in_graph_entity:
        temp_cypher = "CREATE (%s {NAME:'%s'})" % (head, head) + "\n"
    ...
    for relation, tail in relation_data[head].items():
        ...
        if tail not in in_graph_entity:
            temp_cypher = "CREATE (%s {NAME:'%s'})" % (tail, tail) + "\n"
            cypher += "CREATE (%s {NAME:'%s'})" % (tail, tail) + "\n"
            in_graph_entity.add(tail)

        #关系语句
        temp_cypher = "CREATE (%s)-[:%s]->(%s)" % (head, relation, tail) + "\n"
