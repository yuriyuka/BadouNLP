...
bert = BertModel.from_pretrained(r"N:\八斗\上一期\第六周 语言模型\bert-base-chinese", return_dict=False)
...
class DiyBert:
    ...
    def count_parameters(self):
        def param_count(name, param):
            count = np.prod(param.shape)
            print(f"{name:60}: {count:,} parameters")
            return count

        total_params = 0

        # Embedding 层参数
        total_params += param_count("word_embeddings", self.word_embeddings)
        total_params += param_count("position_embeddings", self.position_embeddings)
        total_params += param_count("token_type_embeddings", self.token_type_embeddings)
        total_params += param_count("embeddings_layer_norm_weight", self.embeddings_layer_norm_weight)
        total_params += param_count("embeddings_layer_norm_bias", self.embeddings_layer_norm_bias)

        # Transformer 层参数
        for i, weights in enumerate(self.transformer_weights):
            names = [
                "query_weight", "query_bias", "key_weight", "key_bias", "value_weight", "value_bias",
                "attention_output_weight", "attention_output_bias", "attention_layer_norm_weight", "attention_layer_norm_bias",
                "intermediate_weight", "intermediate_bias", "output_weight", "output_bias",
                "ff_layer_norm_weight", "ff_layer_norm_bias"
            ]
            print(f"\nLayer {i} parameters:")
            for name, param in zip(names, weights):
                total_params += param_count(f"layer_{i}.{name}", param)

        # Pooler 层参数
        total_params += param_count("pooler_dense_weight", self.pooler_dense_weight)
        total_params += param_count("pooler_dense_bias", self.pooler_dense_bias)
