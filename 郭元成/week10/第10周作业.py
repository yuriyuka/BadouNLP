...
def build_sample(tokenizer, window_size, corpus):
    ...
    x = tokenizer.encode(window, add_special_tokens=False, padding='max_length',
                         truncation=True, max_length=10)  # 将字转换成序号
    y = tokenizer.encode(target, add_special_tokens=False, padding='max_length',
...
def train(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            x, y = build_dataset(batch_size, tokenizer, window_size,
                                 corpus)  # 构建一组训练样本
            if torch.cuda.is_available():
                x, y = x.cuda(), y.cuda()
...
if __name__ == "__main__":
    ...
    train(
        r"F:/Desktop/work_space/badou/八斗课程/week10 文本生成问题/lstm语言模型生成文本/corpus.txt",
        False)
