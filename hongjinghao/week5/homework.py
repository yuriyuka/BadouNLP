...
class KMeansClusterer:  # k均值聚类
    ...
    def point_distance(self, result):
        distances = []
        if len(result) != len(self.center):
            raise Exception("result不是本次cluster计算后的值，或未经过cluster计算")

        for index, item in enumerate(result):
            for point in item:
                distances.append([point, self.__distance(self.center[index], point)])
        return distances
    ...
    def __pick_start_point(self, ndarray, cluster_num):
        ...
        for index in indexes:
            ...
            print(f"index:{index}, list:{ndarray[index].tolist()}, point:{points}")
...
for item in kmeans.point_distance(result):
    print(item)
