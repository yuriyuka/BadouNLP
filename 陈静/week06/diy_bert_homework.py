...
bert = BertModel.from_pretrained(r"C:\Users\cj783\Desktop\AI算法工程师\week6\week6 语言模型和预训练\bert-base-chinese", return_dict=False)
...
def count_parameters(state_dict):
    ...
    for name, param in state_dict.items():
        ...
        print(f"{name:<60} => {param.shape} => {param_count}")
        total += param_count
    print(f"\n BERT模型总参数量为: {total:,} 个")
    return total
