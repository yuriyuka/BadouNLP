...
def main(config):
    ...
    if tuning_tactics == "lora_tuning":
        ...
        task_type = "TOKEN_CLS" if config.get("task_type") == "ner" else "SEQ_CLS"
        peft_config = PromptEncoderConfig(task_type=task_type, num_virtual_tokens=10)
    ...
    for epoch in range(config["epoch"]):
        ...
        for index, batch_data in enumerate(train_data):
            ...
            if config.get("task_type") == "ner":
                # NER任务：计算序列标注损失
                # 展平预测结果和标签
                active_loss = labels.view(-1) != 0  # 忽略padding位置
                active_logits = output.view(-1, config["num_labels"])[active_loss]
