...
class TorchModel(nn.Module):
    def __init__(self,vector_dim,hidden_size,vocab):
        ...
        self.polling = nn.AvgPool1d(vector_dim)
    def forward(self,x,y=None):
        x = self.embedding(x)   # 20 * 10 * 10
        x = self.dropout(x)     # 20 * 10 * 10
        x = x.transpose(1,2)   # 20 * 10 * 10
        x = self.polling(x)    # 20 * 10 * 1
        x = x.squeeze()        # 20 * 10
        x = self.linear1(x)    # 20 * 
        x = self.relu(x)       
        x = self.linear2(x)    
...
def main():
    ...
    # 构建保存路径
    save_path = os.path.join(current_dir, 'week03pollingmodel.bin')
    # 保存词表
...
if __name__ == '__main__':
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # 构建保存路径
    path_model = os.path.join(current_dir, 'week03pollingmodel.bin')
