...
"""
建立网络模型结构
主要实现forward函数，搭建参数x到embeddin、model结构的逐层传递计算，最后映射成分类数（分类模型）
"""
...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        class_num = config["class_num"]  # 在 loader 里面定义类别数
    ...
    def forward(self, x, target=None): # x.shape = [batch_size, seq_len]  其中 seq_len = max_len
        if self.use_bert:
            x = self.encoder(x) # [batch_size, seq_len] -> [batch_size, seq_len, hidden_size](取序列输出)
            # 对于BERT:
            # 输入: [batch_size, seq_len]
            # 输出: tuple (sequence_output, pooler_output)
            ...
            x = self.encoder(x) # [batch_size, seq_len, hidden_size] -> [batch_size, seq_len, hidden_size]
            # 输入: [batch_size, seq_len, hidden_size]
            # 输出: [batch_size, seq_len, hidden_size] (对于CNN等)
            # 或: tuple (序列输出, 隐藏状态) (对于RNN/LSTM/GRU)

        if isinstance(x, tuple): #RNN、LSTM、bert等类的模型会同时返回隐单元向量，我们只取序列结果
            x = x[0]
        ...
        x = self.pooling_layer(x.transpose(1,2)).squeeze() #input shape:(batch_size, sen_len, hidden_size) -转置-> (batch_size, hidden_size, sen_len) -> output shape:(batch_size, hidden_size)

        # 也可以直接使用序列最后一个位置的向量
        # x = x[:, -1, :]

        predict = self.classify(x) #input shape:(batch_size, hidden_size) --乘(hidden_size,class_num)线性层--> output shape:(batch_size, class_num)
