...
class DataGenerator:
    ...
    # 从同一个类别中选取两个样本作为anchor和positive
    # 从不同类别中选取一个样本作为negative
    def random_train_sample(self):
        ...
        # 从锚点类别中随机选择两个样本作为anchor和positive
        anchor, positive = random.sample(self.knwb[anchor_class], 2)

        # 随机选择一个不同的类别作为负样本类别
        negative_classes = [idx for idx in standard_question_index if idx != anchor_class]
        if not negative_classes:
            return self.random_train_sample()
        negative_class = random.choice(negative_classes)

        # 从负样本类别中随机选择一个样本作为negative
        negative = random.choice(self.knwb[negative_class])

        # 返回三元组样本 (anchor, positive, negative)
