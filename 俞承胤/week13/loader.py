...
class BertDataGenerator:
    def __init__(self, data_path, config):
        self.config = config
        self.path = data_path
        self.tokenizer = bert_load_vocab(config["pretrain_model_path"])
        self.sentences = []
    ...
    def bert_encode_sentence(self, text, padding=True):
        input_id = []
        for char in text:
            input_id.append(self.tokenizer.vocab.get(char, self.tokenizer.unk_token_id))
        if padding:
...
#加载字表或词表
def bert_load_vocab(vocab_path):
    return BertTokenizer.from_pretrained(vocab_path)



def bert_load_data(data_path, config, shuffle=True):
