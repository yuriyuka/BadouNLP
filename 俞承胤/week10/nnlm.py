...
class LanguageModel(nn.Module):
    def __init__(self):
        ...
        self.bert = BertModel.from_pretrained(r"D:\workspace\pynlp_workspace\ycy2025\model\bert-base-chinese")
...
def bert_build_sample(tokenizer, window_size, corpus):
    start = random.randint(0, len(corpus) - 1 - window_size)
    end = start + window_size
    window = corpus[start:end]
    target = corpus[start + 1:end + 1]  #输入输出错开一位
    # print(window, target)
    # x = tokenizer.encode(window,max_length=50,padding="max_length")
    # y = tokenizer.encode(target,max_length=50,padding="max_length")
    x = tokenizer.encode(window,add_special_tokens=False)
    y = tokenizer.encode(target,add_special_tokens=False)
    return padding(x, 50), padding(y, 50), mask_padding(x, 50)

def mask_padding(x,maxLength):
    if len(x) >= maxLength:
        return [1] * len(x)
    else:
        return [1] * len(x) + [0] * (maxLength - len(x))

def padding(input_id,maxLength):
    input_id = input_id[:maxLength]
    input_id += [0] * (maxLength - len(input_id))
    return input_id
...
def build_dataset(sample_length, tokenizer, window_size, corpus):
    ...
    for i in range(sample_length):
        ...
        x, y, mask = bert_build_sample(tokenizer, window_size, corpus)
        dataset_x.append(x)
        dataset_y.append(y)
        dataset_mask.append(mask)
    # print("build_dataset x", len(x))
    # print("build_dataset y", len(y))
...
def generate_sentence(openings, model, tokenizer, window_size):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            ...
            x = tokenizer.encode(openings[-window_size:],max_length=50,padding="max_length")
