...
class DialogueSystem:
    ...
    def load(self):
        ...
        self.reread_intents = ["重听", "再说一遍", "没听清", "重复一下"]
        self.reread_response = "好的，我再说一遍："
    ...
    def is_reread_intent(self):
        query = memory['query']
        """判断是否为重听意图"""
        for intent in self.reread_intents:
            if intent in query:
                return True
        return False
    ...
    def nlg(self, memory):
        ...
        memory["history_response"].append(memory["response"])
        return memory
    ...
    def run(self, query, memory):
        ...
        '''
        memory["query"] = query
        if self.is_reread_intent():
            if 'response' not in memory:
                memory['response'] = '暂无对话记录'
            return memory
...
if __name__ == '__main__':
    ...
    memory = {"available_nodes": ["scenario-买衣服_node1"], "history_response": []}  # 用户状态
    while True:
        query = input("请输入：")
        # query = "你好"    
