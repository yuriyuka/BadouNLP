...
class GraphQA:
    def __init__(self):
        self.graph = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", "neo4jneo4j"))
    ...
    def parse_result(self, graph_search_result, answer, info):
        ...
        if "REL" in graph_search_result:
            graph_search_result["REL"] = graph_search_result["REL"].type
    ...
    def query(self, sentence):
        ...
        for templet, cypher, score, answer in templet_cypher_score:
            with self.graph.session() as session:
                # print(cypher)
                graph_search_result = session.run(cypher)
                graph_search_result = [dict(record) for record in graph_search_result]
