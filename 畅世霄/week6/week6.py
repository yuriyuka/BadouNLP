...
bert = BertModel.from_pretrained(r"E:\PycharmProjects\NLPtask\week6   语言模型和预训练\bert-base-chinese", return_dict=False)
...
class DiyBert:
    def __init__(self, state_dict):
        self.num_attention_heads = 12  # 12个注意力头
        self.hidden_size = 768  # 隐藏层维度768
        self.num_layers = 1  # 注意这里的层数要跟预训练config.json文件中的模型层数一致（只有1层Transformer）
        ...
        # 打印参数总量（移动到load_weights之后）
        print(f"Total Parameters: {self.total_params:,}")  # 逗号分隔千位
    ...
    def load_weights(self, state_dict):
        ...
        # 计算embedding部分参数量
        self.total_params += count_params(self.word_embeddings)
        self.total_params += count_params(self.position_embeddings)
        self.total_params += count_params(self.token_type_embeddings)
        self.total_params += count_params(self.embeddings_layer_norm_weight)
        self.total_params += count_params(self.embeddings_layer_norm_bias)
        ...
        for i in range(self.num_layers):
            ...
            # 计算当前层的参数量
            layer_params = (
                    count_params(q_w) + count_params(q_b) +
                    count_params(k_w) + count_params(k_b) +
                    count_params(v_w) + count_params(v_b) +
                    count_params(attention_output_weight) + count_params(attention_output_bias) +
                    count_params(attention_layer_norm_w) + count_params(attention_layer_norm_b) +
                    count_params(intermediate_weight) + count_params(intermediate_bias) +
                    count_params(output_weight) + count_params(output_bias) +
                    count_params(ff_layer_norm_w) + count_params(ff_layer_norm_b)
            )
        ...
        # 计算pooler层参数量
        self.total_params += count_params(self.pooler_dense_weight)
        self.total_params += count_params(self.pooler_dense_bias)
