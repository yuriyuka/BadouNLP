...
class DataGenerator(Dataset):
    ...
    def load(self):
        ...
        print("=== knwb 统计 ===")
        print(f"Total keys (标准问题数): {len(self.knwb)}")
        for key, values in self.knwb.items():
            print(f"Key '{key}' 样本数: {len(values)}")
        return
    ...
    def random_train_sample(self):
        ...
        if not standard_question_index:
            raise ValueError("ERROR: knwb 中没有标准问题！请检查数据加载是否正确。")

        max_attempts = 100  # 最大尝试次数，防止死循环
        attempts = 0
