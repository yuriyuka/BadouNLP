...
def evaluate(model):
    ...
    correct, wrong = 0, 0
    with torch.no_grad():
        output = model(x)  # 模型预测 model.forward(x)
...
def main():
    ...
    # 保存模型
    torch.save(model.state_dict(), "model.mul")
    # 画图
    print(log)
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        # print("输入：%s, 预测类别：%d, 概率值：%f" % (vec, int(res), res))  # 打印结果
