...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            return self.random_train_sample()  # 训练集返回三元组
        else:
            return self.data[index]  # 测试集不变
    ...
    def random_train_sample(self):
        ...
        # 随机选一个标准问题，生成正样本 anchor 和 positive
        p = random.choice(standard_question_index)
        if len(self.knwb[p]) < 2:
            return self.random_train_sample()  # 不足两个，重新采样
        anchor, positive = random.sample(self.knwb[p], 2)
        # 负样本从不同标签中选一个
        negative_label = random.choice([x for x in standard_question_index if x != p])
