...
class GraphQA:
    def __init__(self):
        self.graph = Graph("http://localhost:7474", auth=("neo4j", "bado"))
        schema_path = "kg_schema.json"
        templet_path = "question.xlsx"
    ...
    def parse_result(self, graph_search_result, answer, info):
        if len(graph_search_result) >= 2:
            names = [item["item.NAME"] for item in graph_search_result]
            return "、".join(names)
...
if __name__ == "__main__":
    graph = GraphQA()
    print(graph.query("红狼全名叫什么"))
    print(graph.query("露娜叫什么？"))
    print(graph.query("露娜有什么技能"))
    print(graph.query("威龙使用什么武器"))
