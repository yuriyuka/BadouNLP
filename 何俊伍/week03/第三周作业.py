...
基于pytorch的网络编写
实现一个网络完成一个简单nlp任务
构造随机包含a的长度为6的字符串，使用rnn进行多分类，类别为a第一次出现在字符串中的位置。
...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.rnn = nn.RNN(vector_dim, 32, bias=True, batch_first=True)  #RNN层
    ...
    def forward(self, x, y=None):
        ...
        x = self.rnn(x)                           #(batch_size, vector_dim, sen_len)->(batch_size, vector_dim, 1)
...
def build_sample(vocab, sentence_length):
    ...
    for i in range(sentence_length):
        if i == a_index:
            ...
            x.append(random.choice(list(vocab.keys()).replace('a', '')))
    return ''.join(x)
