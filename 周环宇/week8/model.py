...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, anchor, positive=None, negative=None, target=None):
        # 如果传入了 positive 和 negative，则认为是训练三元组输入
        if positive is not None and negative is not None:
            a_vec = self.sentence_encoder(anchor)  # (batch_size, hidden_size)
            p_vec = self.sentence_encoder(positive)  # (batch_size, hidden_size)
            n_vec = self.sentence_encoder(negative)  # (batch_size, hidden_size)

            # 使用自定义的 cosine triplet loss
            loss = self.cosine_triplet_loss(a_vec, p_vec, n_vec, target)
            return loss

        # 向下兼容：如果只传入 sentence1 和 sentence2，则保持原来的行为
