...
class DialogueSystem:
    def __init__(self):
        self.load()
        self.rerequest_pattern = re.compile(r'(再说一遍|重复一下|没听清|没听懂|重新说|什么)')  # 重听意图的正则模式
    ...
    def is_rerequest(self, query):
        """判断是否为重听请求"""
        return bool(self.rerequest_pattern.search(query))
    ...
    def intent_judge(self, memory):
        ...
        # 先检查是否为重听请求
        if self.is_rerequest(query) and memory.get('last_response'):
            memory["is_rerequest"] = True
            return memory

        memory["is_rerequest"] = False
        max_score = -1
        hit_node = None
    ...
    def nlg(self, memory):
        if memory["policy"] == "rerequest":
            # 重听请求，返回上一轮的回复
            memory["response"] = memory.get("last_response", "抱歉，我没有之前的对话记录")
