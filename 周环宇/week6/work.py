...
state_dict = bert.state_dict() #获取 BERT 模型的所有参数（权重和偏置）并存储在一个字典中
bert.eval() #将模型设置为评估模式，这会影响一些层（如 Dropout 和 BatchNorm）的行为。
x = np.array([2450, 15486, 102, 2110])   #假想成4个字的句子
torch_x = torch.LongTensor([x])          #pytorch形式输入，输出格式为 tensor([2450, 15486, 102, 2110])
...
class DiyBert:
    ...
    def count_parameters(self):
        ...
        # 计算Embedding层参数
        total_params += np.prod(self.word_embeddings.shape)  # word_embeddings
        total_params += np.prod(self.position_embeddings.shape)  # position_embeddings
        total_params += np.prod(self.token_type_embeddings.shape)  # token_type_embeddings
        total_params += np.prod(self.embeddings_layer_norm_weight.shape)  # LayerNorm weight
        total_params += np.prod(self.embeddings_layer_norm_bias.shape)  # LayerNorm bias

        # 计算Transformer层参数
        for layer_weights in self.transformer_weights:
            for weight in layer_weights:
                total_params += np.prod(weight.shape)

        # 计算Pooler层参数
        total_params += np.prod(self.pooler_dense_weight.shape)  # dense weight
        total_params += np.prod(self.pooler_dense_bias.shape)  # dense bias

        return total_params

    def load_weights(self, state_dict):
        #embedding部分
        self.word_embeddings = state_dict["embeddings.word_embeddings.weight"].numpy() #词表embedding，依据预训练模型文档，应为一个21128*786的矩阵
        self.position_embeddings = state_dict["embeddings.position_embeddings.weight"].numpy() #位置embedding层，应为一个文本长度*768的矩阵
        self.token_type_embeddings = state_dict["embeddings.token_type_embeddings.weight"].numpy() #语句标注层标注为第一句还是第二句，依据文档应为一个2*768的矩阵
        self.embeddings_layer_norm_weight = state_dict["embeddings.LayerNorm.weight"].numpy() #Layer Normalization层归一化层权重参数
...
# 计算并输出参数总量
total_params = db.count_parameters()
print(f"模型的可训练参数总量: {total_params:,}")
print(f"约等于: {total_params/1e6:.1f}M 参数")
