...
class GraphQA:
    def __init__(self):
        ...
        templet_path = "question_templet.xlsx"  #加载模版
    ...
    def parse_sentence(self, sentence):
        entitys = self.get_mention_entitys(sentence) #如果说法灵活经常用没见过的表述方式来说用ner，如是固定的名称适合用正则来提取
    ...
    def query(self, sentence):
        ...
        for templet, cypher, score, answer in templet_cypher_score: #每个模板会有分数
            graph_search_result = self.graph.run(cypher).data()    #按分数去找最高的
            # 最高分命中的模板不一定在图上能找到答案, 当不能找到答案时，运行下一个搜索语句, 找到答案时停止查找后面的模板
            if graph_search_result:
                answer = self.parse_result(graph_search_result, answer, info) #把结果作下包装再返回出来
                return answer        
        return None
...
if __name__ == "__main__":
    graph = GraphQA()
    res = graph.query("范特西的发行时间是什么")
    print(res)
    res = graph.query("爱我别走的歌手是谁")
    print(res)
    res = graph.query("发布时间在1990-2000所有的歌曲")
    print(res)
