...
class TorchModel(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        # x = self.embedding(x)  #input shape:(batch_size, sen_len)batch_size批次大小，sen_len 是序列长度。将输入 x 通过嵌入层转换为词向量表示
        # x, _ = self.layer(x)      #input shape:(batch_size, sen_len, hidden_size * 2) 将嵌入后的词向量 x 输入到双向LSTM层。_ 表示LSTM层的隐藏状态和细胞状态，这里被忽略
        # predict = self.classify(x) #ouput:(batch_size, sen_len, num_tags) -> (batch_size * sen_len, num_tags)将双向LSTM的输出 x 通过线性分类层映射到标签空间
