...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        self.bert = BertModel.from_pretrained(config["bert_path"], return_dict=False) #bert包含embedding层
...
def main(config):
    ...
    if tuning_tactics == "lora_tuning":
        peft_config = LoraConfig(
            r=8,
            lora_alpha=32,  # 放缩的参数默认32就行
            lora_dropout=0.1,
            target_modules=["query", "key", "value"]  # 很重要！决定在哪部分用这个lora
        )
