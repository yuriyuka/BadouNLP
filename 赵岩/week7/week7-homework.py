# 课程作业：使用 BERT / LSTM / CNN 在电商评论数据集上做文本分类试验
...
def tokenize(texts, max_len=64):
    # 指定本地路径
    tokenizer = BertTokenizer.from_pretrained('E:/BaiduNetdiskDownload/week6 语言模型和预训练/bert-base-chinese')
...
class BERTClassifier(nn.Module):
    def __init__(self, hidden_size=128, num_classes=2):
        super().__init__()
        # 使用本地路径加载预训练模型
        self.bert = BertModel.from_pretrained('E:/BaiduNetdiskDownload/week6 语言模型和预训练/bert-base-chinese')
...
class LSTMClassifier(nn.Module):
    ...
    def forward(self, x, *args):  # ignore attention_mask
        x = self.embedding(x)
        out, _ = self.lstm(x)
