...
规律：x是一个5维向量，最大值所在的维度即为该向量的类别标签
...
# 随机生成一个5维向量，最大值所在的维度即为该向量的类别标签
def build_sample():
    x = np.random.random(5)
    label = np.argmax(x)  # 最大值所在的维度即为类别标签
    return x, label
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        wrong += (y_pred != y).sum().item()
    print("本次预测集中共有%d个样本，正确预测个数：%d, 正确率：%f" % (test_sample_num, correct, correct / (correct + wrong)))
