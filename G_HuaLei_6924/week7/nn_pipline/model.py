...
class TorchModel(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        if self.pooling_style == "max":
            ...
            print(f'self.pooling_layer: {self.pooling_layer}')
        else:
...
class CNN(nn.Module):
    ...
    def forward(self, x): #x : (batch_size, max_len, embeding_size)  ==> 卷积层要求输入的数据seq_len放在后面，即 max_len放在后面，如果是2D 或者 3D ，则其他维度的数据也放在后面
