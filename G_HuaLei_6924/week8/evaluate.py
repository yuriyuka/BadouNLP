    ...
    def knwb_to_vector(self):
        ...
        for standard_question_index, question_ids in self.train_data.dataset.knwb.items():
            for question_id in question_ids:
                ...
                # 所有扩展问中，每一个扩展问句子汇总词label组成的向量对应的标准问的 label
        ...
        with torch.no_grad():
            ...
            #  将所有的问题用最新训练的模型参数重新向量化，用来重新测试对比与新的测试提问是否是同一中提问
            self.knwb_vectors = self.model(question_matrixs) # 返回的是 size*hidden_size 的批量句子张量，每一个向量代表一个句子
            #将所有向量都作归一化 v / |v|
