...
规律：改用交叉熵实现一个多分类任务，五维随机向量最大的数字在哪维就属于哪一类（维度类别（0-4））
...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.activation = torch.tanh  # nn.Sigmoid() sigmoid归一化函数
...
def evaluate(model):
    ...
    print("本次预测集中各类别样本数量：", np.bincount(y.numpy()))
    correct = 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测 model.forward(x)
        pred = torch.argmax(y_pred, dim=1)  # 获取预测类别
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        ...
        print("输入：%s, 预测类别：%d, 概率值：%s" % (vec, pred_class, res.numpy()))  # 打印结果
