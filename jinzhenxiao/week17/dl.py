...
class DialogueSystem:
    ...
    def nlg(self, memory):
        ...
        if memory.get("is_replay", False):
            if "history" in memory and memory["history"]:
                memory["response"] = memory["history"][-1]  # 返回最后一次回复
            else:
                memory["response"] = "抱歉，没有历史对话记录可以重听。"
            return memory
    ...
    def is_replay_request(self, query):
        # 检测是否为重听请求
        replay_keywords = ["重听", "再说一遍", "再说一次", "没听清", "重复一下", "你说什么"]
    ...
    def run(self, query, memory):
        ...
        if "history" not in memory:
            memory["history"] = []
        if memory.get("response") and not memory.get("is_replay", False):
            memory["history"].append(memory["response"])
