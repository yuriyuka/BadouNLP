...
class GraphQA:
    def __init__(self):
        self.graph = Graph("http://localhost:7474", auth=("neo4j", "aaa"))
        schema_path = "new_kg_schema.json"
        templet_path = "new_question_templet.xlsx"
...
if __name__ == "__main__":
    graph = GraphQA()
    res = graph.query("谁导演的复仇者联盟")
    print(res)
    res = graph.query("谁饰演的美国队长")
    print(res)
    res = graph.query("钢铁侠的真实姓名是什么")
    print(res)
    res = graph.query("美国队长的超能力是什么")
    print(res)
    res = graph.query("钢铁侠和复仇者联盟是什么关系")
    print(res)
