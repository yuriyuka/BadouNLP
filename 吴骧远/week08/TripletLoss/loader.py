...
class DataGenerator:
    ...
    #生成triplet训练样本
    #随机选择一个问题类别，从中选择两个样本作为anchor和positive
    #然后从其他类别中选择一个样本作为negative
    def random_train_sample(self):
        standard_question_index = list(self.knwb.keys())
        # 随机选择anchor的类别
        anchor_class = random.choice(standard_question_index)
        # 如果选中的类别下样本数量不足2个，重新选择
        if len(self.knwb[anchor_class]) < 2:
            return self.random_train_sample()
        # 从anchor类别中选择两个样本：anchor和positive
        anchor, positive = random.sample(self.knwb[anchor_class], 2)
        # 从其他类别中选择negative样本
        other_classes = [cls for cls in standard_question_index if cls != anchor_class]
