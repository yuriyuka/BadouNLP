...
# Self Attention vs Feed Forward参数量对比
total_self_attention = 0
total_feed_forward = 0

for i in range(12):
    layer_self_attention = 0
    layer_feed_forward = 0
    ...
    for component in attention_components:
        if component in state_dict:
            layer_self_attention += state_dict[component].numel()

    for component in ff_components:
        if component in state_dict:
            layer_feed_forward += state_dict[component].numel()

    total_self_attention += layer_self_attention
    total_feed_forward += layer_feed_forward

print(f"Self Attention总参数: {total_self_attention:,} ({total_self_attention / 1e6:.2f}M)")
print(f"Feed Forward总参数: {total_feed_forward:,} ({total_feed_forward / 1e6:.2f}M)")
print(f"Feed Forward是Self Attention的 {total_feed_forward / total_self_attention:.2f} 倍")

if total_feed_forward > total_self_attention:
    print("Feed Forward模块参数量更多")
else:
    print("Self Attention模块参数量更多")
