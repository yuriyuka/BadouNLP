...
class DataGenerator:
    def __init__(self, data_path, config):
        ...
        if self.config["model_type"] == "bert":
            ...
            self.vocab = tokenizer.vocab  # 词表字典：{token: id}
    ...
    def encode_sentence(self, text, padding=True):
        ...
        if self.use_bert == True:
            for char in text:
                input_id.append(self.vocab.get(char, self.vocab["[UNK]"])) # 根据bert词典逐字获取idx  不能直接用bert的分词
        elif self.config["vocab_path"] == "words.txt":
