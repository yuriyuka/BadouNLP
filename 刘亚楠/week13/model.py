...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        if self.model_type == "lstm":
            ...
            classify_hidden_size = hidden_size * 2
        elif self.model_type == "bert":
            self.use_bert = True
            self.layer = BertModel.from_pretrained(config["bert_path"], return_dict=False)
            classify_hidden_size = self.encoder.config.hidden_size
        elif self.model_type == "bert_mid_layer":
            self.use_bert = True
            self.layer = BertMidLayer(config)
            classify_hidden_size = self.layer.bert.config.hidden_size
