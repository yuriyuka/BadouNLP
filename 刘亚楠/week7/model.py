...
class TorchModel(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        if isinstance(x, tuple):  #RNN类的模型会同时返回隐单元向量，我们只取序列结果
            # PyTorch的RNN层在forward方法中会返回两个值：output和h_n。其中，output是每个时间步的隐藏状态，而h_n是最后一个时间步的隐藏状态。
            # RNN的返回值包括output和h_n，其中output的形状是(seq_len, batch, num_directions * hidden_size)，而h_n是最后一层的隐藏状态
            x = x[0]
        #可以采用pooling的方式得到句向量
