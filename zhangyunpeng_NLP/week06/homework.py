...
'''通过手动矩阵运算实现Bert结构: 模型文件下载 https://huggingface.co/models'''
...
# print(state_dict.keys())  #查看所有的权值矩阵名称
...
class DiyBert:
    ...
    def __init__(self, state_dict):
        self.num_attention_heads = 12  # 12个注意力机制头
        self.hidden_size = 768  # 768维 = 12个注意力头 * 每个头64维 <-- 提高并行效率    ??[768] --如何切分--> [12 * 64] (Q、K、V)
        self.num_layers = 12  # 等于config.json的模型层数:  bert-base的num_layers=12 bert-large的num_layers=24
        self.load_weights(state_dict)

    def load_weights(self, state_dict):
        # embedding部分
        self.word_embeddings = state_dict[
            "embeddings.word_embeddings.weight"].numpy()  # 词表长度, bert-base-chinese词表长度为21128‌‌
        self.position_embeddings = state_dict["embeddings.position_embeddings.weight"].numpy()  # 长度最大为512
        self.token_type_embeddings = state_dict["embeddings.token_type_embeddings.weight"].numpy()  # 位置编码(0, 1)长度：2
        self.embeddings_layer_norm_weight = state_dict["embeddings.LayerNorm.weight"].numpy()
        self.embeddings_layer_norm_bias = state_dict["embeddings.LayerNorm.bias"].numpy()
        self.transformer_weights = []
        # transformer部分，有12层
        for i in range(self.num_layers):  # Q、K、V 向量  权重和偏置
            ...
            attention_output_weight = state_dict[
                "encoder.layer.%d.attention.output.dense.weight" % i].numpy()  # softmax(Q、K、V矩阵计算)后的权重
            attention_output_bias = state_dict[
                "encoder.layer.%d.attention.output.dense.bias" % i].numpy()  # softmax(Q、K、V矩阵计算)后的偏置
            attention_layer_norm_w = state_dict[
                "encoder.layer.%d.attention.output.LayerNorm.weight" % i].numpy()  # 归一化后的权重
            attention_layer_norm_b = state_dict[
                "encoder.layer.%d.attention.output.LayerNorm.bias" % i].numpy()  # 归一化后的偏置
            intermediate_weight = state_dict["encoder.layer.%d.intermediate.dense.weight" % i].numpy()  # 全连接层的权重
            intermediate_bias = state_dict["encoder.layer.%d.intermediate.dense.bias" % i].numpy()  # 全连接层的偏置
            ...
            ff_layer_norm_w = state_dict["encoder.layer.%d.output.LayerNorm.weight" % i].numpy()  # 归一化后的权重
            ff_layer_norm_b = state_dict["encoder.layer.%d.output.LayerNorm.bias" % i].numpy()  # 归一化后的偏置
    ...
    # 计算bert-base参数
    def calc_parameter_numbers(self):
        # bert-base 12层transformer
        # 1、embedding层：word embedding + segment embedding + position embeing = 21128‌‌ + 512 + 2
        embedding_para_num = self.hidden_size * (21128 + 512 + 2)
        print(f'embedding层的参数有{embedding_para_num}个')
        # 2、transformer层
        self_attention_para_num = (self.hidden_size * (self.hidden_size / 12) * 3) * 12  # 注意力机制中：Q、K、V相关计算
        fnn_para_num = self.hidden_size * self.hidden_size  # 前馈神经网络
        transf_para_num = (self_attention_para_num + fnn_para_num) * self.num_layers
        print(f'transformer层的参数有{transf_para_num}个, 其中自注意力机制的参数有{self_attention_para_num}个')
        # 3、全连接层
        fnn_para_num = 2 * self.hidden_size * (self.hidden_size * 4) * self.num_layers + self.hidden_size * (
                    self.hidden_size * 4)
        print(f'全连接层的参数有{fnn_para_num}个')
        # 4、LayerNorm(weight、bias): embedding层后、multi-head attention后、feed forward后
        norm_para_num = 2 * self.hidden_size + (2 * self.hidden_size) * 12 + (2 * self.hidden_size) * 12
        print(f'LayerNorm层的参数有{norm_para_num}个')
        # 5、基于预训练过程的Masked LM和NSP（next sentence prediction）的参数
        masked_and_nsp_para_num = 2 * self.hidden_size + self.hidden_size * self.hidden_size
        print(f'关于Masked LM和NSP的参数有{masked_and_nsp_para_num}个')

        all_para_num = embedding_para_num + transf_para_num + fnn_para_num + norm_para_num + masked_and_nsp_para_num
        print(f'bert-base的总参数有{all_para_num}个')
        return all_para_num
