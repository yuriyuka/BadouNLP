    ...
    with open(path, 'r', encoding='utf-8') as f:
        for line in f:
            if line.strip():
                ...
                data.append({
                    'question': item['title'],  # 标题作为问题
                    'answer': item['content']   # 内容作为答案
                })
    return data
...
def build_qa_sample(tokenizer, data_item, max_length=128):
    ...
    # 构造输入格式: [CLS] question [SEP] answer [SEP]
    # 但训练时我们只用问题作为输入，答案作为输出
    ...
    answer_tokens = tokenizer.encode(
        answer,
        add_special_tokens=False,  # 不添加特殊token，因为我们要连接到问题后
        truncation=True,
        max_length=max_length//2
    )
    ...
    # 构建标签序列，问题部分用0填充(不计算损失)，答案部分保留真实token
...
def train(data_path, save_weight=True):
    ...
    # 预训练模型路径
    pretrain_model_path = r'D:\workspace\pretrain_models\bert-base-chinese'
