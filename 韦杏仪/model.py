...
class SiameseNetwork(nn.Module):
    ...
    def loss_triplet(self, anchor_vector, positive_vector, negative_vector):
    ...
    def forward(self, sentence1, sentence2=None, target=None):
        ...
        if sentence2 is not None:
            if target is not None and target.shape[1] == 1:
                ...
                vector2 = self.sentence_encoder(sentence2)
                return self.loss(vector1, vector2, target.squeeze())
            elif isinstance(sentence2, torch.Tensor) and isinstance(target, torch.Tensor):
                anchor_vector = self.sentence_encoder(sentence1)
                positive_vector = self.sentence_encoder(sentence2)
                negative_vector = self.sentence_encoder(target)
                return self.loss_triplet(anchor_vector, positive_vector, negative_vector)
        else:
            return self.sentence_encoder(sentence1)
