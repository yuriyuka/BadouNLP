...
#训练Word2Vec模型
def train_word2vec_model(sentences, model_path="word2vec.model"):
    # 将句子转换为词列表
    word_lists = []
...
def sentences_to_vectors(sentences,model):
    vectors=[]
    for sentence in sentences:
        words=sentence.split()  #sentence是分好词的，用空格分开
        ...
        for word in words:
            try:
                ...
                #部分词在训练中未出现，用全0向量代替
                vector+=np.zeros(model.vector_size)
        vectors.append(vector/len(words))  #除以词数，求平均
    return np.array(vectors)

def main():
    model_path = "word2vec.model"
    
    # 尝试加载现有模型，如果不存在则训练新模型
    model = load_word2vec_model(model_path)
    if model is None:
        print("开始训练Word2Vec模型...")
        sentences = load_sentence("titles.txt")  #加载所有标题
        model = train_word2vec_model(sentences, model_path)
    else:
        sentences = load_sentence("titles.txt")  #加载所有标题
    
    vectors = sentences_to_vectors(sentences, model)   #将所有标题向量化

    n_clusters=int(math.sqrt(len(sentences)))  #聚类数目，取平方根
    print("聚类数目：", n_clusters)
    #Kmeans聚类
    kmeans= KMeans(n_clusters=n_clusters, random_state=42)#指定一个kmeans计算类
    kmeans.fit(vectors)  #聚类

    sentence_label_dict=defaultdict(list)  #句子与聚类标签的对应关系
    for sentence,label in zip(sentences,kmeans.labels_):#取出标签和句子
        sentence_label_dict[label].append(sentence)   #同标签的放到一起

        #计算类内距离
    density_dict=defaultdict(list)  #类内距离
    for vector_index,label in enumerate(kmeans.labels_):
        vector=vectors[vector_index]  #某句话的向量
        center=kmeans.cluster_centers_[label]  #该类的中心
        distance=cosine_distance(vector, center)  #余弦距离
        density_dict[label].append(distance)  #累加距离
