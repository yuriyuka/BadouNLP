...
class TextCNNModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab, output_dim=7):
        ...
        self.rnn1 = nn.RNN(vector_dim, vector_dim, bias=False, batch_first=True)
    ...
    def forward(self, x, y=None):
        ...
        x, _ = self.rnn1(x)  # (batch_size, sen_len, vector_dim) -> (batch_size, vector_dim, sen_len)
        x = x.permute(0, 2, 1)
        x = self.pool(x)  #(batch_size, vector_dim, sen_len)->(batch_size, vector_dim, 1)
        x = x.squeeze()  #(batch_size, vector_dim, 7) -> (batch_size, vector_dim)
