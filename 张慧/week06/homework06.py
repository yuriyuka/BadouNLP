...
bert_model_path = project_root / 'week06/bert-base-chinese'
print(bert_model_path)

# bert_model_path = '../week06/bert-base-chinese'
...
class DiyBert:
    ...
    def layer_norm(self, x, w, b):
        # 1. 计算输入x在每行(axis=1)上的均值，并保持维度(keepdims=True)
        # 2. 计算输入x在每行上的标准差
        # 3. 对x进行标准化：(x - 均值)/标准差
    ...
    def calculate_embedding_parmeters(self):
        # 计算embedding层的参数量
        # 参数量 = 词汇表大小 × 嵌入维度
        vocab_size = self.word_embeddings.shape[0]  # 词汇表大小
        word_embeddings_params = vocab_size * self.hidden_size  # word_embeddings嵌入矩阵的参数量

        max_position_embeddings = 512  # 最大位置编码数
        # 参数量 = 最大位置编码数 × 嵌入维度
        position_embeddings_params = max_position_embeddings * self.hidden_size  # position_embeddings嵌入矩阵的参数量

        # 参数量 =  token类型数 × 嵌入维度
        token_type_embeddings = 2  # token类型数
        token_type_embeddings_params = token_type_embeddings * self.hidden_size  # token type嵌入矩阵的参数量

        # LayerNorm层的参数量
        embeddings_layer_norm_params = self.embeddings_layer_norm_weight.size + self.embeddings_layer_norm_bias.size

        # 总参数量
        embeddings_total_params = (word_embeddings_params + position_embeddings_params +
                        token_type_embeddings_params + embeddings_layer_norm_params)

        return embeddings_total_params

    def calculate_single_transformer_parameters(self, layer_index):
        # 计算单层transformer的参数量
        # 注意力机制参数量
        weights = self.transformer_weights[layer_index]
        # 取出该层的参数，在实际中，这些参数都是随机初始化，之后进行预训练
        ...
        # 注意力机制参数量
        attention_params = (q_w.size + q_b.size + k_w.size + k_b.size +
                            v_w.size + v_b.size + attention_output_weight.size +
                            attention_output_bias.size + attention_layer_norm_w.size +
                            attention_layer_norm_b.size+intermediate_weight.size + intermediate_bias.size +
                            output_weight.size + output_bias.size + ff_layer_norm_w.size +
                            ff_layer_norm_b.size)
        return attention_params

    def calculate_all_transformer_parameters(self):
        all_attention_params = 0
        for i in range(self.num_layers):
            all_attention_params += self.calculate_single_transformer_parameters(i)
        return all_attention_params

    def calculate_pooler_parameters(self):
        # 计算pooler层的参数量
        pooler_dense_params = self.pooler_dense_weight.size + self.pooler_dense_bias.size
        return pooler_dense_params

    def calculate_total_parameters(self):
        # 计算所有层的参数量
        embeddings_params = self.calculate_embedding_parmeters()
        all_attention_params = self.calculate_all_transformer_parameters()
        pooler_params = self.calculate_pooler_parameters()
        total_params = embeddings_params + all_attention_params + pooler_params
        return total_params
...
total_params = db.calculate_total_parameters()
print("============ DiyBert总参数量=============")
print(total_params)
