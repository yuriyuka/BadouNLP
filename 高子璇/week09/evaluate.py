...
class Evaluator:
    ...
    def decode(self, sentence, labels):
        """
        sentence: str，原始句子
        labels  : list[int]，对齐到 BERT sub-word
        返回实体字典
        """
        # 先把 label 转成字符串
        ...
        for m in re.finditer('(04+)', label_str):
            s, e = m.span()
            results["LOCATION"].append(sentence[s:e])
        for m in re.finditer('(15+)', label_str):
            s, e = m.span()
            results["ORGANIZATION"].append(sentence[s:e])
        for m in re.finditer('(26+)', label_str):
            s, e = m.span()
            results["PERSON"].append(sentence[s:e])
        for m in re.finditer('(37+)', label_str):
            s, e = m.span()
