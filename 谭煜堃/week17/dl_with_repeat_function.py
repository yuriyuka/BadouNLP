...
class DialogueSystem:
    ...
    def is_repeat_query(self, text):
        if not text: return False
        pat = r"(没听清|听不清|再说(一遍|一次)?|重复|重听|重说|重讲|请再说|能再说一遍|repeat|你说啥|啥|什么|什么呀|没听懂|听不懂|听不清楚)"
        return re.search(pat, str(text), re.IGNORECASE) is not None
    ...
    def nlg(self, memory):
        ...
        if memory["policy"] == "reply":
            ...
            #policy == "request"
            # 重听功能（repeat function
            if self.is_repeat_query(memory["query"]):
                # 重听功能：重复之前的响应，并添加前缀
                previous_response = memory.get("response", "")
                memory["response"] = "好的，现在重新说一遍：" + previous_response
            else:
