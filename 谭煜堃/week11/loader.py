...
class DataGenerator:
    ...
    def prepare_data(self, title, content):
        input_seq = self.encode_sentence(title, self.config["input_max_length"], False, False) #输入序列
        output_seq = self.encode_sentence(content, self.config["output_max_length"], True, False) #输出序列

        gold = self.encode_sentence(content, self.config["output_max_length"], False, True) #不进入模型，用于计算loss
