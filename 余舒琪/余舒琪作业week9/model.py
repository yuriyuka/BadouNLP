...
class TorchModel(nn.Module):
    ...
    def forward(self, x, target=None, attention_mask=None):
        if self.use_bert:
            x, _ = self.layer(x, attention_mask=attention_mask)
        else:
        ...
        if target is not None:
            if self.use_crf:
                mask = target.gt(-1) 
                return - self.crf_layer(predict, target, mask, reduction="mean") #计算所有样本损失的平均值
            else:
                #(number, class_num), (number)
                return self.loss(predict.view(-1, predict.shape[-1]), target.view(-1))
        else:
            if self.use_crf:
                return self.crf_layer.decode(predict) #用的是维特比解码
            else:
                return predict
