...
class GraphQA:
    def __init__(self):
        self.graph = Graph("http://localhost:7474", auth=("neo4j", "neo4j977"))
    ...
    def get_mention_entities(self, sentence):
    ...
    def parse_sentence(self, sentence):
        entities = self.get_mention_entities(sentence)
...
if __name__ == "__main__":
    graph = GraphQA()
    res = graph.query("上汽集团的股东是谁")
    print(res)
    res = graph.query("谁收购了联创汽车电子公司")
    print(res)
    res = graph.query("华域汽车的供应商是")
    print(res)
    res = graph.query("福田汽车的总部所在地是")
    print(res)
    res = graph.query("国机智骏破产时间是")
    print(res)
    res = graph.query("国机汽车的总部所在地")
    print(res)
    res = graph.query("众泰汽车和湖南江南汽车制造有限公司重庆分公司是什么关系")
    print(res)
