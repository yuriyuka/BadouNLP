...
class DialogueSystem:
    ...
    def nlg(self, memory):
        ...
        # 新增功能：判断是否需要重听
        if 'repeat' in memory and memory['repeat']:
            memory["response"] = "好的，我重新问一遍："+ memory.get("last_response", memory["response"])

        # 存储当前的回复，以便下次重听
        memory["last_response"] = memory["response"].replace("好的，我重新问一遍：","")
        return memory
    ...
    def run(self, query, memory):
        ...
        memory["query"] = query
        # 检查是否有重听的请求
        if "重听" in query or "再来一遍" in query or "没听清" in query or "说什么" in query:
            memory["repeat"] = True  # 如果用户请求重听，设置flag
        else:
            memory["repeat"] = False  # 否则设置为False
