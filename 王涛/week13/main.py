...
def main(config):
    ...
    if tuning_tactics == "lora_tuning":
        peft_config = LoraConfig(lora_alpha=32, lora_dropout=0.1, target_modules=["query", "key", "value"])
    elif tuning_tactics == "p_tuning":
        peft_config = PromptEncoderConfig(task_type="TOKEN_CLS", num_virtual_tokens=30)
    elif tuning_tactics == "prompt_tuning":
        peft_config = PromptTuningConfig(task_type="TOKEN_CLS", num_virtual_tokens=30)
    elif tuning_tactics == "prefix_tuning":
        peft_config = PrefixTuningConfig(task_type="TOKEN_CLS", num_virtual_tokens=30)
    ...
    if tuning_tactics == "lora_tuning":
        ...
        for param in model.get_submodule("model").get_submodule("classify").parameters(): #对于其他模型结构，或许可以用更智能的查找方式
