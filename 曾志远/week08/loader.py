...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            ...
            return self.random_train_sample_triplet()
        else:
    ...
    def random_train_sample_triplet(self):
        standard_question_index = list(self.knwb.keys())
        pa, n = random.sample(standard_question_index, 2)
        # 如果选取到的标准问下不足两个问题，则无法选取，所以重新随机一次
        if len(self.knwb[pa]) < 2 and len(self.knwb[n]) < 2:
            return self.random_train_sample_triplet()
        else:
            if len(self.knwb[pa]) >= 2:
                p, a = random.sample(self.knwb[pa], 2)
                n = random.choice(self.knwb[n])
                return [a, p, n]
            else:
                p, a = random.sample(self.knwb[n], 2)
                n = random.choice(self.knwb[pa])
                return [a, p, n]
