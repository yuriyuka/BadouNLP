...
class DialogueSystem:
    ...
    def updateRewriteResponse(self, memory):
        response = memory["response"]
        for key, node in self.all_node_info.items():
            if key in memory["rewriteNode"]:
    ...
    def run(self, query, memory):
        ...
        # 重听机制
        self.updateRewriteResponse(memory)
        return memory
...
if __name__ == '__main__':
    ...
    memory = {"available_nodes": ["scenario-买衣服_node1"], "rewriteNode": ["scenario-买衣服_node5"]}  #用户状态
    while True:
        query = input("请输入：")
        # query = "你好"
