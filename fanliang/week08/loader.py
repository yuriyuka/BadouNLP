...
class DataGenerator:
    ...
    #正样本从随机一个标准问题中随机选取两个
    def random_train_sample1(self):
        standard_question_index = list(self.knwb.keys())
        #随机正样本，用positive_sample_rate控制正负样本比例
    ...
    def random_train_sample(self):
        ...
        if len(self.knwb[p]) < 2:
            return self.random_train_sample()
        action, positive = random.sample(self.knwb[p], 2)
        nagetive = random.choice(self.knwb[n])
        return [action, positive, nagetive]
