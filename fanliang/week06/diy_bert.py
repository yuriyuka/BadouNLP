...
bert = BertModel.from_pretrained(current_working_dir+"/models/bert-base-chinese", return_dict=False)
...
class DiyBert:
    ...
    def self_attention(self,
        ...
        #[num_attention_heads, max_len, attention_head_size]*[num_attention_heads, attention_head_size, max_len] = [num_attention_heads, max_len, max_len]
        # qk.shape = num_attention_heads, max_len, max_len 最后两维度max_len*max_len代表每一个与任何其他字的关联度，元素数量为max_len*max_len
        qk = np.matmul(q, k.swapaxes(1, 2))  
        # qk /= np.sqrt(attention_head_size) 除以一个缩放因子，避免qk过大，导致softmax饱和
        qk = softmax(qk)
        #[num_attention_heads, max_len, max_len]*[num_attention_heads, max_len, attention_head_size] = [num_attention_heads, max_len, attention_head_size]
