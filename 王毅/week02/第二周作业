...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        #y_pred = self.activation(x)  # (batch_size, 1) -> (batch_size, 5)
        if y is not None:
            y = torch.argmax(y, dim=1)  # 将one-hot转类别索引
            return self.loss(y_pred, y.long())  # 预测值和真实值计算损失
        else:
            return torch.argmax(y_pred,dim=1)  # 输出预测结果

# 生成一个样本, 定义一个样本的生成方法，代表了我们要学习的规律
# 随机生成一个5维向量，这个5维向量视为样本X，如果某一维度的值最大，则对应维度的真实值为1，其他值为0
def build_sample():
...
def evaluate(model):
    ...
    # 获取唯一值并转为列表
    unique_classes = torch.unique(class_indices).tolist()
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if y_p == np.argmax(y_t): # 如果两个五维向量是相等的，则认为，预测值准确
                correct += 1
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    for vec, res, prob in zip(input_vec, result, probs):
        print(f"输入：{vec}, 预测类别：{res.item()}, 概率值：{prob[res].item():.4f}")
