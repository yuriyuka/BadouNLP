...
class RNNModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab, dropout_rate, rnn_hidden_size):
        ...
        self.linear = nn.Sequential(  # 线性层
            nn.Linear(vector_dim, 1024),  # (batch_size, hidden_size) -> (batch_size, 1024)
    ...
    def forward(self, x, y=None):
        ...
        y_pred = self.linear(x)  # (batch_size, vector_dim) -> (batch_size, sentence_len + 1)
        if y is not None:
