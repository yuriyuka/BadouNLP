...
bert = BertModel.from_pretrained(r"D:\八斗NLP课件\week06 语言模型和预训练\bert-base-chinese", return_dict=False)
...
print('diy_sequence_output is:\n', diy_sequence_output)
print('torch_sequence_output is:\n', torch_sequence_output)

# print(diy_pooler_output)
# print(torch_pooler_output)

# 计算bert模型总参数量
def count_bert_parameters(state_dict, num_layers):
    total_params = 0

    # 1. 统计Embedding层参数
    embeddings_params = 0
    embeddings_params += np.prod(state_dict["embeddings.word_embeddings.weight"].shape)  # 词嵌入
    embeddings_params += np.prod(state_dict["embeddings.position_embeddings.weight"].shape)  # 位置嵌入
    embeddings_params += np.prod(state_dict["embeddings.token_type_embeddings.weight"].shape)  # 类型嵌入
    embeddings_params += np.prod(state_dict["embeddings.LayerNorm.weight"].shape)  # LN权重
    embeddings_params += np.prod(state_dict["embeddings.LayerNorm.bias"].shape)  # LN偏置
    print(f"Embeddings 层参数: {embeddings_params:,}")
    total_params += embeddings_params

    # 2. 统计Transformer层参数（按层统计）
    for i in range(num_layers):
        layer_params = 0
        # 注意力部分
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.self.query.weight"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.self.query.bias"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.self.key.weight"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.self.key.bias"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.self.value.weight"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.self.value.bias"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.output.dense.weight"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.output.dense.bias"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.output.LayerNorm.weight"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.attention.output.LayerNorm.bias"].shape)
        # 前馈部分
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.intermediate.dense.weight"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.intermediate.dense.bias"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.output.dense.weight"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.output.dense.bias"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.output.LayerNorm.weight"].shape)
        layer_params += np.prod(state_dict[f"encoder.layer.{i}.output.LayerNorm.bias"].shape)

        print(f"Transformer 层 {i + 1} 参数: {layer_params:,}")
        total_params += layer_params

    # 3. 统计Pooler层参数
    pooler_params = 0
    pooler_params += np.prod(state_dict["pooler.dense.weight"].shape)
    pooler_params += np.prod(state_dict["pooler.dense.bias"].shape)
    print(f"Pooler 层参数: {pooler_params:,}")
    total_params += pooler_params

    return total_params


num_layers = 1  # 此处与config.json中的num_hidden_layers保持一致
total_params = count_bert_parameters(bert.state_dict(), num_layers)
print(f"\nBERT模型层数:{num_layers}, 总参数量为: {total_params}")
