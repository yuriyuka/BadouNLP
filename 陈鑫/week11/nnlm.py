...
def get_valid_token_ids(tokenizer):
    """返回所有不以##开头且非特殊符号的token id"""
    valid = []
    for idx, token in enumerate(tokenizer.get_vocab()):
        if not token.startswith("##") and token not in {"[UNK]", "[PAD]", "[CLS]", "[SEP]"}:
            valid.append(idx)
    return torch.tensor(valid, dtype=torch.long, device="cuda" if torch.cuda.is_available() else "cpu")


@torch.no_grad()
def generate_sentence(prompt, model, tokenizer, max_len=256):
    model.eval()
    ...
    while True:
        curr = token_ids[-max_len:]
        input_tensor = torch.tensor([curr], device=device)
        attn = (input_tensor != tokenizer.pad_token_id).long()
        probs = model(input_tensor, attention_mask=attn)[0, -1]

        # 只保留合法 token 的概率，重新归一化
        probs = probs[valid_ids]
        probs /= probs.sum()

        # 采样
        if random.random() < 0.1:
            next_id = valid_ids[torch.multinomial(probs, 1).item()].item()
        else:
            next_id = valid_ids[torch.argmax(probs)].item()

        if next_id == tokenizer.sep_token_id or len(token_ids) >= max_len:
            break
