...
class SentenceEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        #使用线性层
        x = self.layer(x)
        """ 最大池化，将32*20*128的张量，池化成了32*128的张量 """
...
class SiameseNetwork(nn.Module):
    def __init__(self, config):
        ...
        if config["loss_type"] == "TripletLoss":
            self.loss = nn.TripletMarginWithDistanceLoss(margin=0.1)
            # self.loss = nn.TripletMarginLoss(margin=0.1, p=2)
        else:
