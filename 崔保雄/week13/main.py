...
def main(config):
    ...
    if config["use_lora"] == True:
        model_path = os.path.join(config["model_path"], f"ner_{config["model_type"]}_lora_epoch{config["epoch"]}.pth")
...
if __name__ == "__main__":
    ...
    """ 以下是进行模型训练，或者预测 """
    is_train = False #是训练模型，还是使用训练好的模型预测
    if is_train:
        """ 训练模型 """
