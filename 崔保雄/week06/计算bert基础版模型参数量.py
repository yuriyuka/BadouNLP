...
#计算模型参数大小(本次作业用到的方法)
def model_param_size(model_state_dict):
    size = 0;
    for key in model_state_dict:
        matrix = model_state_dict[key].numpy()
        shapes = list(matrix.shape);
        shape_size = 1;
        for shape in shapes:
...
""" 加载bert模型 """
bert = BertModel.from_pretrained(r"D:\www.root\bert-base-chinese", return_dict=False)
state_dict = bert.state_dict()

modelsize = model_param_size(state_dict)
print("模型参数大小modelsize：", modelsize)

#BERT 模型中的.eval()函数主要用于在模型评估阶段改变模型的行为，以确保模型以最佳状态运行。如禁用‌梯度计算等
bert.eval()
x = np.array([2450, 15486, 102, 2110])   #假想成4个字的句子
torch_x = torch.LongTensor([x])          #pytorch形式输入

"""
    最后隐藏状态seqence_output的形状为 (batch_size, sequence_length, hidden_size)，其中：
        batch_size是输入样本的数量。
        sequence_length是输入句子的长度。
        hidden_size是BERT模型隐层的大小，通常是768。
"""
seqence_output, pooler_output = bert(torch_x)
print("随意假象4个句子调用：\n", "seqence_output.shape：", seqence_output.shape, "pooler_output.shape：", pooler_output.shape)
# print(seqence_output, pooler_output)

print("bert.state_dict().keys()：\n", bert.state_dict().keys())  #查看所有的权值矩阵名称
...
print(f"diy_sequence_output，sharp：{diy_sequence_output.shape}：\n", diy_sequence_output)
print(f"torch_sequence_output，sharp：{torch_sequence_output.shape}：\n\n",torch_sequence_output)
