...
老师点评：
    1、二分类:输出0-1之间的一个数 大于0.5正类，小于0.5负类
    2、多分类:输出概率分布，要分几类就输出几维的向量【0.1,0.1,0.1,0.5,0.2】
    3、二分类也可以输出2维向量 用交叉熵损失函数【0.1,0.9】
"""
...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        """
            这里不需要再次指定softmax激活函数，是因为cross_entropy交叉熵自带了softmax。
            加上self.activation = torch.softmax这一行，也没任何影响。
        """
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            """
            以下这两行都可以，不加softmax返回的预测值可能有负值。
            """
            #return y_pred  # 输出预测结果
            return torch.softmax(y_pred, axis = -1)  # 输出预测结果
