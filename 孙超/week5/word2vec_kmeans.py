...
def cal_avg_distances(vectors, labels, cluster_centers):
...
def main():
    ...
    # 计算类内平均距离
    avg_distances = cal_avg_distances(vectors, kmeans.labels_, kmeans.cluster_centers_)

    sentence_label_dict = defaultdict(list)
    for sentence, label in zip(sentences, kmeans.labels_):  #取出句子和标签
        sentence_label_dict[label].append(sentence)         #同标签的放到一起

    sorted_clusters = sorted(avg_distances.items(), key=lambda x: x[1], reverse=True)

    for label, avg_distance in sorted_clusters:
        print("cluster %s, 平均距离: %s:" % (label, avg_distance))
