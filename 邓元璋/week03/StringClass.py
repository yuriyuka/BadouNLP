...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.classify = nn.Linear(6, 6)  # 线性层，输入维度修改为RNN的输出维度
        self.activation = nn.Softmax(dim=1)  # 修改：使用Softmax作为分类激活函数
        self.loss = nn.CrossEntropyLoss()  # 修改：使用交叉熵损失函数

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.embedding(x)  # (batch_size, sen_len) -> (batch_size, sen_len, vector_dim)
        rnn_output, _ = self.layer(x)  # (batch_size, sen_len, 6)
        # 修改：使用最后一个时间步的输出作为特征
        x = rnn_output[:, -1, :]  # (batch_size, 6)
        x = self.classify(x)  # (batch_size, 6)
        y_pred = self.activation(x)  # (batch_size, 6)，每个位置的概率分布
        if y is not None:
            # 修改：适应交叉熵损失函数的输入格式
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        ...
        print(f"输入：{input_string}, 预测类别：{predicted_class}, 概率分布：{result[i].numpy()}")  # 打印结果
