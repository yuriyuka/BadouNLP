...
class DialogueSystem:
    ...
    def run(self, query, memory):
        ...
        '''
        # 检查是否为重听指令
        if query in ["重听", "再说一遍", "重复"]:
            if "history" in memory and len(memory["history"]) > 0:
                # 使用上一次的回复
                memory["response"] = memory["history"][-1]
            else:
                memory["response"] = "之前没有对话内容可以重听。"
            return memory
...
if __name__ == '__main__':
    ...
    memory = {
        ...
        "history": []  # 用于存储对话历史，支持重听功能
    }

    print("欢迎使用对话系统！输入'重听'可以重复上一条回复，输入'退出'结束对话。")
    while True:
        query = input("请输入：")
        if query == "退出":
            print("对话结束，再见！")
