...
# 随机生成一个5维向量，第几位最大就是第几类
def build_sample():
    x = np.random.random(5)  # 生成5维随机向量
    y = np.zeros(5)  # 创建全0的5维向量
    max_index = np.argmax(x)  # 获取最大值的索引(0-4)
    y[max_index] = 1  # 将最大值对应的位置设为1
    return x, y  # 返回输入向量和one-hot编码的标签向量
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        print("y sample:", y[0])
        pred_class = torch.argmax(y_pred, dim=1)  # 预测类别索引
        true_class = torch.argmax(y, dim=1)  # 实际类别索引
...
def predict(model_path, input_vec):
    ...
    for i in range(len(input_vec)):
        print(f"样本{i}: 预测类别={pred_classes[i]}, 概率={[f'{p:.2f}' for p in probs[i]]}")  # 打印结果
