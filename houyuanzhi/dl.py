...
class DialogueSystem:
    def __init__(self):
        self.load()
        self.dialogue_history = []  # 添加对话历史记录
    ...
    def nlg(self, memory):
        ...
        if memory["policy"] == "reply":
            ...
            if len(self.dialogue_history) > 0:
                memory["response"] = self.dialogue_history[-1]
            else:
                memory["response"] = "这是对话的开始，没有可以重听的内容。"
        else:
            #policy == "request"
    ...
    def run(self, query, memory):
        ...
        '''
        # 检查是否是重听指令
        if query.strip() in ["重听", "再听一遍", "repeat"]:
        ...
        # 将本轮对话响应添加到历史记录中
        self.dialogue_history.append(memory["response"])
        # 保持历史记录在合理长度内
